...
regions_dict = (
    data_dir + "mike_working_dir/identifying_regions_data/adjusted_regions.pkl"
)
regions_stat_dict = (
    data_dir
    + "mike_working_dir/identifying_regions_data/twins_era_stats_dict_radius_regions_min_2.pkl"
)
    ...
    for file in twins_files:
        ...
        for i, date in enumerate(twins_map["Epoch"]):
            ...
            check = pd.to_datetime(
                date.strftime(format="%Y-%m-%d %H:%M:%S"), format="%Y-%m-%d %H:%M:%S"
            )
            if check in times.values:
                maps[check.round("T").strftime(format="%Y-%m-%d %H:%M:%S")] = {}
                maps[check.round("T").strftime(format="%Y-%m-%d %H:%M:%S")][
                    "map"
                ] = twins_map["Ion_Temperature"][i][50:140, 40:100]
...
def loading_supermag(station):
    ...
    df.index = pd.to_datetime(df.index, format="%Y-%m-%d %H:%M:$S")
    df["theta"] = (
        np.arctan2(df["N"], df["E"]) * 180 / np.pi
    )  # calculates the angle of B_H
    df["cos_theta"] = np.cos(
        df["theta"] * np.pi / 180
    )  # calculates the cosine of the angle of B_H
    df["sin_theta"] = np.sin(
        df["theta"] * np.pi / 180
    )  # calculates the sine of the angle of B_H
...
def combining_stations_into_regions(
    stations,
    rsd,
    features=None,
    mean=False,
        ...
        for feature in features:
            if mean:
                if feature == "N" or feature == "E":
                    regional_df[f"{feature}_mean"] = (
                        feature_dfs[feature].abs().mean(axis=1)
                    )
                else:
            ...
            if maximum:
                if feature == "N" or feature == "E":
                    regional_df[f"{feature}_max"] = (
                        feature_dfs[feature].abs().max(axis=1)
                    )
                else:
                    regional_df[f"{feature}_max"] = feature_dfs[feature].max(axis=1)
            if median:
                if feature == "N" or feature == "E":
                    regional_df[f"{feature}_median"] = (
                        feature_dfs[feature].abs().median(axis=1)
                    )
                else:
                    regional_df[f"{feature}_median"] = feature_dfs[feature].median(
                        axis=1
                    )
    ...
    regional_df["rolling_rsd"] = (
        rsd["max_rsd"]["max_rsd"].rolling(indexer, min_periods=1).max()
    )
...
def splitting_and_scaling(
    ...
    if dates is not None:
        ...
        x_train, x_val, y_train, y_val, dates_train, dates_val = train_test_split(
            x_train, y_train, dates_train, test_size=val_size, random_state=random_seed
        )

        dates_dict = {"train": dates_train, "test": dates_test, "val": dates_val}
    else:
        x_train, x_test, y_train, y_test = train_test_split(
            input_array, target_array, test_size=test_size, random_state=random_seed
        )
    ...
    if scaling_method == "standard":
        ...
        raise ValueError(
            'Must specify a valid scaling method for TWINS. Options are "standard" and "minmax".'
        )
...
def classification_column(df, param, thresh, forecast, window):
    ...
    df[f"shifted_{param}"] = df[param].shift(
        -forecast
    )  # creates a new column that is the shifted parameter. Because time moves foreward with increasing
    ...
    if (
        ...
        indexer = pd.api.indexers.FixedForwardWindowIndexer(
            window_size=window
        )  # Yeah this is annoying, have to create a forward rolling indexer because it won't do it automatically.
        df["window_max"] = (
            df[f"shifted_{param}"].rolling(indexer, min_periods=1).max()
        )  # creates new column in the df labeling the maximum parameter value in the forecast:forecast+window time frame
    ...
    df["classification"] = np.select(
        conditions, binary
    )  # new column created using the conditions and the binary
    ...
    df.drop(
        ["window_max", f"shifted_{param}"], axis=1, inplace=True
    )  # removes the working columns for memory purposes
...
def storm_extract(
    ...
    if twins and map_keys is None:
        ...
        storm_list = pd.DataFrame(
            {
                "dates": [
                    pd.to_datetime(key, format="%Y-%m-%d %H:%M:%S") for key in map_keys
                ]
            }
        )
        storm_list = storm_list["dates"]
    ...
    stime, etime = (
        [],
        [],
    )  # will store the resulting time stamps here then append them to the storm time df
    ...
    for start, end in zip(
        storm_list["stime"], storm_list["etime"]
    ):  # looping through the storms to remove the data from the larger df
        if (
            start < df.index[0] or end > df.index[-1]
        ):  # if the storm is outside the range of the data, skip it
            continue
    ...
    if concat:
        for storm, tar in zip(storms, y):
            ...
            )  # resetting the storm index and simultaniously dropping the date so it doesn't get trained on
            all_targets = pd.concat(
                [all_targets, pd.DataFrame(tar)], axis=0, ignore_index=True
            )
...
def split_sequences(
    ...
    index_to_drop = 0
    for sequence, target, twins in zip(
        sequences, targets, maps
    ):  # looping through the sequences and targets
        for i in range(len(sequence) - n_steps):  # going to the end of the dataframes
            ...
            if include_target:
                if np.isnan(seq_x).any():
                    if (
                        dates is not None
                    ):  # doesn't add arrays with nan values to the training set
                ...
                if model_type == "classification":
                    ...
                    raise ValueError(
                        'Must specify a valid model type. Options are "classification" and "regression".'
                    )
                y.append(seq_y1)
            X.append(seq_x)
            if maps is not None:
