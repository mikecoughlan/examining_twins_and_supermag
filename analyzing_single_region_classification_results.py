...
def attaching_mlt(predictions, region):
	...
	for station in stations:
		...
		latitudes.append(temp_df['MLAT'].iloc[0])
...
def prediction_error_vs_MLT(all_predictions):
	...
	for region in all_predictions.keys():
		...
		plt.scatter(predictions['MLT'], error, s=10, label=region)
	plt.legend()
	ax.set_xlabel('Actual Values')
	ax.set_ylabel('Predictions')
	ax.set_title('actual-predicted vs MLT')
	ax.plot([0, 1], [0, 1], transform=ax.transAxes, ls='--', c='r')

	plt.savefig(f'plots/{TARGET}/analysis_plots_modeling_v{VERSION}/simple_scatter_plot_with_latitude.png')
